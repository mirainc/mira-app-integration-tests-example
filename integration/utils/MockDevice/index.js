...
class MockDevice extends EventEmitter {
  constructor({ autoplay = false } = {}) {
    ...
    if (this.shouldAutoplay) this.on(PRESENTATION_READY, this.play);
    if (this.shouldAutoplay) this.on(PRESENTATION_COMPLETE, this.play);
  }
  ...
  autoplay = (shouldAutoplay = true) => (this.shouldAutoplay = shouldAutoplay);
